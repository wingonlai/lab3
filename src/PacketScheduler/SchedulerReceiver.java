	...
	{		
		...
		try
		{
			...
			while (true)
			{
				...
				int Priority = (int)packet.getData()[0];
				...
				// add differnent priority packet to different queues if there are enough space
				if(Priority == 1){
					if (buffers[0].addPacket(new DatagramPacket(packet.getData(), packet.getLength())) < 0)
					{
						System.err.println("Packet dropped (Priority 1 queue full).");
						packetDroped ++;
					}
				}
				else if (Priority == 2){
					...
					{
						System.err.println("Packet dropped (Priority 2 queue full).");
						packetDroped ++;
					}
				}
				else if (Priority == 3){
					...
					{
						System.err.println("Packet dropped (Priority 3 queue full).");
						packetDroped ++;
					}
				}
				else
					System.out.println("You should not be here, this is a bug of Priority setting.");

				pOut.print(packetDroped +"\t");
				pOut.println();
